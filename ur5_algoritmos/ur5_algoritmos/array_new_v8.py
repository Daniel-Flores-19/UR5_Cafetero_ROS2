#!/usr/bin/env python3
"""
array_new_v7.py — Publicador ROS2 de trayectorias de letras usando pipeline V7 A4.
...
Notas V7:
    - Agrega retroceso global sobre trazos ya escritos.
    - Permite postergar puntos/tildes/diéresis al final.
    - Evita unir marcas superiores al cuerpo de la letra con líneas falsas.
"""
...
from std_msgs.msg import Float32MultiArray, Int8MultiArray, MultiArrayDimension, Bool

# En ROS2 debe importar desde el paquete. El fallback permite probarlo localmente
# si array_new_v7.py y letter_trajectory_functions_v7.py están en la misma carpeta.
try:
    from ur5_algoritmos.letter_trajectory_functions_v7 import (
        run_full_pipeline,
        A4_WIDTH_MM,
        A4_HEIGHT_MM,
        PX_PER_MM,
        ROI_MM as DEFAULT_ROI_MM,
        ROI_MARGIN_MM as DEFAULT_ROI_MARGIN_MM,
        GLOBAL_RETRACE_ENABLED,
        GLOBAL_RETRACE_ATTACH_MAX_DISTANCE,
        GLOBAL_RETRACE_MAX_ROUTE_PIXELS,
        GLOBAL_RETRACE_CONNECTOR_POINTS,
        GLOBAL_RETRACE_ALLOW_MARKS,
        POSTPONE_MARKS_TO_END,
        AVOID_MERGING_MARKS,
    )
except ImportError:  # pragma: no cover - solo para pruebas fuera del paquete ROS
    from letter_trajectory_functions_v7 import (
        run_full_pipeline,
        A4_WIDTH_MM,
        A4_HEIGHT_MM,
        PX_PER_MM,
...
# letter_trajectory_functions_v7.py usará una fuente TrueType de respaldo.
...
# Nuevas variables V7
GLOBAL_RETRACE_ENABLED_DEFAULT = GLOBAL_RETRACE_ENABLED
GLOBAL_RETRACE_ATTACH_MAX_DISTANCE_DEFAULT = GLOBAL_RETRACE_ATTACH_MAX_DISTANCE
GLOBAL_RETRACE_MAX_ROUTE_PIXELS_DEFAULT = GLOBAL_RETRACE_MAX_ROUTE_PIXELS
GLOBAL_RETRACE_CONNECTOR_POINTS_DEFAULT = GLOBAL_RETRACE_CONNECTOR_POINTS
GLOBAL_RETRACE_ALLOW_MARKS_DEFAULT = GLOBAL_RETRACE_ALLOW_MARKS
POSTPONE_MARKS_TO_END_DEFAULT = POSTPONE_MARKS_TO_END
AVOID_MERGING_MARKS_DEFAULT = AVOID_MERGING_MARKS
...
def a4_image_size_from_px_per_mm(px_per_mm: float) -> tuple[int, int]:
    """Calcula el tamaño de lienzo A4 en píxeles usando escala uniforme."""
    return (
        int(round(A4_WIDTH_MM * float(px_per_mm))),
        int(round(A4_HEIGHT_MM * float(px_per_mm))),
    )
...
class LetterTrajectoryPublisher(Node):
    """Genera una palabra con pipeline V7 y publica la trayectoria como arrays."""
    ...
    def _on_image_done(self, msg: Bool):
    ...
    def _start_pipeline(self):
        trajectories_xy, _, _, _ = self._generate_trajectories()
    ...
    def _declare_parameters(self) -> None:
        ...
        self.declare_parameter("roi_margin_mm", float(DEFAULT_ROI_MARGIN_MM))
        ...
        # Parámetros nuevos del V7
        self.declare_parameter("global_retrace_enabled", GLOBAL_RETRACE_ENABLED_DEFAULT)
        self.declare_parameter("global_retrace_attach_max_distance", GLOBAL_RETRACE_ATTACH_MAX_DISTANCE_DEFAULT)
        self.declare_parameter("global_retrace_max_route_pixels", GLOBAL_RETRACE_MAX_ROUTE_PIXELS_DEFAULT)
        self.declare_parameter("global_retrace_connector_points", GLOBAL_RETRACE_CONNECTOR_POINTS_DEFAULT)
        self.declare_parameter("global_retrace_allow_marks", GLOBAL_RETRACE_ALLOW_MARKS_DEFAULT)
        self.declare_parameter("postpone_marks_to_end", POSTPONE_MARKS_TO_END_DEFAULT)
        self.declare_parameter("avoid_merging_marks", AVOID_MERGING_MARKS_DEFAULT)
    ...
    def _load_parameters(self) -> None:
        ...
        self.roi_margin_mm = float(self.get_parameter("roi_margin_mm").value)
        ...
        self.global_retrace_enabled = bool(self.get_parameter("global_retrace_enabled").value)
        self.global_retrace_attach_max_distance = int(self.get_parameter("global_retrace_attach_max_distance").value)
        self.global_retrace_max_route_pixels = int(self.get_parameter("global_retrace_max_route_pixels").value)
        self.global_retrace_connector_points = int(self.get_parameter("global_retrace_connector_points").value)
        self.global_retrace_allow_marks = bool(self.get_parameter("global_retrace_allow_marks").value)
        self.postpone_marks_to_end = bool(self.get_parameter("postpone_marks_to_end").value)
        self.avoid_merging_marks = bool(self.get_parameter("avoid_merging_marks").value)
        ...
        self.margin_px = int(round(self.roi_margin_mm * self.px_per_mm))
        self.image_size = a4_image_size_from_px_per_mm(self.px_per_mm)
    ...
    # ------------------------------------------------------------------
    # Pipeline V7 completo
    # ------------------------------------------------------------------

    def _generate_trajectories(self):
        """
        Ejecuta run_full_pipeline() de V7 y convierte splines A4 mm a XY robot.
        """
        ...
        self.get_logger().info(
            ...
            f"margen={self.roi_margin_mm:.2f} mm"
        )
        ...
        splines, spline_arrays_mm, data, graph_info = run_full_pipeline(
            ...
            global_retrace_enabled=self.global_retrace_enabled,
            global_retrace_attach_max_distance=self.global_retrace_attach_max_distance,
            global_retrace_max_route_pixels=self.global_retrace_max_route_pixels,
            global_retrace_connector_points=self.global_retrace_connector_points,
            global_retrace_allow_marks=self.global_retrace_allow_marks,
            postpone_marks_to_end=self.postpone_marks_to_end,
            avoid_merging_marks=self.avoid_merging_marks,
    ...
    def _log_pipeline_stats(self, splines, data, graph_info) -> None:
        ...
        total_retraces = sum(int(s.get("retrace_count", 0)) for s in splines)
        total_global_retraces = sum(int(s.get("global_retrace_count", 0)) for s in splines)
        ...
        self.get_logger().info(
            f"Caminos: esqueleto={len(data.get('raw_skeleton_paths', []))}, "
            f"marcas={len(data.get('mark_paths', []))}, "
            f"ordenados={len(data.get('ordered_paths', []))}, "
            f"finales={len(data.get('final_paths', []))}"
        )
        self.get_logger().info(
            f"Splines generados: {len(splines)} | "
            f"retrocesos locales/globales: {total_retraces}/{total_global_retraces}"
        )
