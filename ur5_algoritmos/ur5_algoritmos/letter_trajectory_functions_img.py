...
def filter_canny_by_density(
    edges: np.ndarray,
    window_size: int = 31,
    density_threshold: float = 0.15,
    suppression_mode: str = "thin",
) -> np.ndarray:
    """
    Suppresses edges in locally dense regions of a Canny edge map.

    Parameters
    ----------
    edges            : binary edge map from auto_canny_limits (0/255, single channel)
    window_size      : size of the local neighbourhood used to measure density (px).
                       Larger = coarser density estimate. Should be odd.
    density_threshold: fraction of edge pixels in the local window above which
                       a region is considered dense (0.0–1.0).
                       0.10–0.20 works well for most photos.
    suppression_mode : "thin"   — keep only the skeleton of dense-region edges
                                  (preserves contour shape, removes parallel/fill edges)
                       "dilate" — keep edges near local maxima of density gradient
                                  (preserves the most prominent boundary in each cluster)
                       "drop"   — remove all edges in dense regions entirely
                                  (most aggressive, fastest)

    Returns
    -------
    filtered edges map (same shape and dtype as input)
    """
    assert window_size % 2 == 1, "window_size must be odd"

    edge_bin = (edges > 0).astype(np.float32)

    # Local density = fraction of edge pixels in a square neighbourhood
    kernel = np.ones((window_size, window_size), dtype=np.float32) / (window_size ** 2)
    density = cv2.filter2D(edge_bin, -1, kernel) 

    dense_mask = density > density_threshold  
    sparse_mask = ~dense_mask

    if suppression_mode == "drop":
        result = edges.copy()
        result[dense_mask] = 0
        return result

    elif suppression_mode == "thin":
        from skimage.morphology import skeletonize as ski_skeletonize
        dense_edges = (edge_bin * dense_mask.astype(np.float32)).astype(bool)
        thinned = ski_skeletonize(dense_edges)
        result = edges.copy()
        result[dense_mask] = 0
        result[thinned] = 255
        return result

    elif suppression_mode == "dilate":
        density_smooth = cv2.GaussianBlur(density, (window_size, window_size), 0)
        local_max = cv2.dilate(density_smooth, np.ones((5, 5), np.uint8))
        prominence_mask = (density_smooth >= local_max - 1e-6) & dense_mask
        result = edges.copy()
        result[dense_mask] = 0
        result[prominence_mask & (edge_bin > 0)] = 255
        return result
