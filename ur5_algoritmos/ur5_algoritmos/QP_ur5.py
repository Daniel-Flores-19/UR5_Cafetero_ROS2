...
PHASE_CIRCLE     = 3  # dibujando el círculo (forward)
...
class UR5ControlNode(Node):
    ...
    def __init__(self):
        ...
        self.radius = 0.08
        self.omega  = 0.5
        self.center = np.array([-0.7, 0.1, 0.177])
    ...
    def force_cb(self, msg: WrenchStamped):
        """Lee el sensor de fuerza"""
        try:
            
            self.fuerza_z = msg.wrench.force[2]
            print(self.fuerza_z)
    ...
    def go_to_circle_start(self):
        ...
        # IK para obtener q objetivo
        # (ajusta según tu función IK; aquí uso una genérica)
        # Si no tienes IK analítica, puedes hacer un pequeño QP convergente offline
        # o simplemente enviar la posición articular actual + un pequeño desplazamiento
        # Por ahora usamos el QP iterativo para convergir al punto de inicio:
    ...
    def update(self):
        ...
        if self.phase == PHASE_GOTO_START:
            ...
            success = self.go_to_circle_start()
            if not success:
            ...
            self.phase = PHASE_CIRCLE
            print(self.phase)
            self.get_logger().info("¡Iniciando trayectoria circular!")
            return

        # ── FASE 3: círculo con forward_position_controller ───────
        if self.phase == PHASE_CIRCLE:
