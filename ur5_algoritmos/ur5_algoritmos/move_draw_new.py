...
class UR5ControlNode(Node):
    ...
    def __init__(self):
        ...
        self.drawing_done_pub = self.create_publisher(Bool, "image_drawing/done", 10)
    ...
    def update(self):
        ...
        if self.phase == PHASE_DRAW:
            ...
            if np.linalg.norm(error) < self.limit_error:
                ...
                if self.punto_goal >= len(self.points):
                    ...
                    # Reset state machine for the next drawing run
                    self.points_received = False
                    self.temp = 1
                    self.cont = 0
                    self.punto_goal = 0
                    ...
                    done_msg = Bool()
                    done_msg.data = True
                    self.drawing_done_pub.publish(done_msg)
                    return
        ...
        if self.phase == PHASE_UP:
    	   ...
    	   # -------------------------------------------------------------------------
                ...
                dq = compute_dq_qp(
            fkine=fkine_ur5, jacobian_func=numerical_jacobian, TF2xyzquat=TF2xyzquat,
            q=self.q, xd=xd, K=2.0, lamb=self.lamb, dt=self.dt
            )
                self.q += np.clip(dq, -2.0, 2.0) * self.dt
